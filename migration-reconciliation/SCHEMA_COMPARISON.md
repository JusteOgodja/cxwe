...
SUPERSEDED **2** · **PROVEN_NOT_APPLIED 0** · UNKNOWN **8**. (Total 15.)

> **Correction terminologique (importante)** : on écrit **`PROVEN_NOT_APPLIED = 0`** (et non
> `NOT_APPLIED = 0`). **La présence d'une table ne prouve pas que la migration correspondante a
> été appliquée intégralement. Les colonnes, contraintes, index, fonctions, grants et policies
> doivent être comparés.** `PROVEN_NOT_APPLIED = 0` signifie seulement qu'aucune migration n'a été
> *prouvée absente* ; les **8 UNKNOWN** restent non prouvées dans un sens comme dans l'autre tant
> que la comparaison de schéma détaillée (staging) n'est pas faite.
